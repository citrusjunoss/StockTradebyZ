...
def _load_industry_from_stocklist(
    ...
    """从 stock_info_cache 读取行业，返回 ['代码','行业']"""
    ...
    if not industries:
        return pd.DataFrame(columns=['代码', '行业'])
...
def compute_j_industry_distribution(
    *,
    ...
    for code, df_code in frames.items():
        ...
        kd = compute_kdj(df)
        j_map[code] = float(
            kd.iloc[-1]["J"]) if (kd is not None and not kd.empty) else float("nan")

    df_j_all = pd.DataFrame(
        {"代码": list(j_map.keys()), "J(日)": list(j_map.values())})

    # 4) 行业
    industry_df = _load_industry_from_stocklist(codes)
    ...
    industry_counts = (
        selected_all["行业"].value_counts().rename_axis(
            "行业").reset_index(name="股票数")
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", type=str,
                        default="./data", help="历史数据目录")
    parser.add_argument("--j_threshold", type=float,
                        default=15.0, help="J(日) 阈值")
    ...
    """保存缓存到文件"""
    with open(Path('./cache/industry/cache.json'), 'w', encoding='utf-8') as f:
